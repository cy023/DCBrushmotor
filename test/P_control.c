...
int main(void)
{
    ...
    //P_gain = 1.28787
    PIDStr_t.PID_gain.Kp = 1.28787;
    PIDStr_t.PID_gain.Ki = 0.0001;
    ...
    while (1) {
        //PIDStr_t.PID_error.i_err += (PIDStr_t.Cmd - (float)ADCresult);
        if ((PIDStr_t.Cmd - (float)ADCresult) <= 0)
            PIDStr_t.FB = -40 * (PIDStr_t.PID_gain.Kp  * (PIDStr_t.Cmd - (float)ADCresult) - 10) / 1003 + 60;
        else
            PIDStr_t.FB = -40 * (PIDStr_t.PID_gain.Kp  * (PIDStr_t.Cmd - (float)ADCresult) + 10) / 1003 + 40;
        
        PIDStr_t.PID_error.d_err =  ((PIDStr_t.Cmd - (float)ADCresult) - PIDStr_t.PID_lesterror.d_lerr) / 0.064;
        
        // PIDStr_t.FB = PIDStr_t.FB + PIDStr_t.PID_gain.Ki*PIDStr_t.PID_error.i_err;
        PIDStr_t.PID_lesterror.d_lerr = (PIDStr_t.Cmd - (float)ADCresult);
